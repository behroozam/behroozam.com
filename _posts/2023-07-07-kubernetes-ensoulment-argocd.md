...
قبل از کوبرنتیز هم ارکستریتر های دیگه ای وجود داشتن من شخصا طرفدار رنچر بودم ولی خیلی ها هم از سوارم استفاده میکردن. 
زیرساخت سنتی تر مبتنی بر baremetal یا vm  بود که عملا ایزوله کردن اپلیکیشن ها رو از همدیگه رو غیرممکن یا خیلی سخت میکرد. 
...
و با مفهوم پاد برای ایزوله کردن یک یا تعدادی از کانتینرها و استفاده از CRI و  CSI و CNI 
تونست ماژولار باشه و برای تعداد خیلی زیادی از شرکت ها جذاب باشه. 
کامیونتینی که دور کوبرنتیز هم شکل گرفت باعث شد که این lifescycle توسعه نرم افزار متحول بشه چیزی که امروز بهش میگیم cloud native applications محصول این تحوله. 
این بلوغ و شکوفایی کوبرنتیز هنوز به انتهای خودش نرسیده 
دور این اکوسیستم ابزارهای مسیریابی و لاگ و سنجه و استقرار و .... 
شکل گرفته که امروز میخوام درمورد یکی از محبوب ترین ابزارهای استقرار یعنی ArgoCD صحبت کنم. یکم درمورد معماریش و اینکه چطور کار میکنه و بعد درمورد اینکه چطوری میشه یه نسخه HA و تر و تمیز ازش بالا اورد و البته سناریو های مختلفی که ممکنه ما باهاشون رو به رو بشیم یا بهشون نیازمند باشیم. 
...
اصولا مفهوم دواپس هم برای کمرنگ کردن این مرز به وجود اومد.
 ...
 در دوران کرتاسه سیس ادمین ها زیرساخت و امنیت و دسترسی رو ایجاد و مدیریت میکردن و شاید اگه استقراری هم درکار بود تو فواصل زمانی خیلی دیر به دیر نرم افزار رو به روزرسانی میکردن حالا به مدل سنتی و با ftp یا sftp یا مدل یکمی آپدیت تر و به وسیله ابزار های source control مثل git یا congfiguration management مثل ansible یا chef. 
...
حالا که درمورد تاریخچه صحبت کردم بزارید درمورد این صحبت کنم که قبل از اینکه ابزارهایی مثل Flux و ArgoCD و Spinnaker وجود داشتن از چه چیزی برای دیپلوی روی کوبرنتیز استفاده میشد. 

همون طور که میدونید مدل کوبرنتیز client و server هستش و resource ها به صورت declarative درخواست میشن اگه این ریسورس معتبر باشه یعنی kube-api و validation webhook ها تشخیص بدن که میشه این ریسورس رو ساخت درنهایت status این ریسورس اپدیت میشه و ساخته میشه در غیر این صورت بسته به اینکه ارور ما معنایی هستش یا لغوی از ساخته شدن ریسورس جلوگیری میشه. 
...
برای ساختن کلاستر های تستی و توسعه کوبرنتیز هیچ چیزی رو ساده تر از k3s نیافتم برای حتی راحت تر کردن ساخت کلاستر های k3s شرکت رنچر که قبلا هم گفته بودم از ابزار های محبوب منه اومده و یه ابزار کوچولو و خیلی خوبی رو ساخته به اسم k3d. 

تصور میکنم که شما از یه سیستم linux استفاده میکنید و docker و kubectl رو هم روی سیستمون نصب دارید 
من خودم الان روی سیستم ویندوزی خودم هستم با HyperV هستم با ubuntu server 22.04. 
 
#### نصب و راه اندازی اولین کلاستر تستی کوبرنتیزمون 
...
احتمالا شما با resource های دیگه کوبرنتیز مثل Deployment و Statefulset هم آشنایی دارید که برای دیپلوی نرم افزار روی کلاستر کوبرنتیز منطقی تر هستند ولی چون هدف این پست بلاگ آموزشی هستش من با همون ساده ترین شکل ریسورس که پاد باشه ادامه میدم و برای اینکه مثال ها در دسترس باشه و بعدا ازشون برای َArgoCD استفاده کنیم یه مخزن روی گیتهاب میسازم. 

##### kustomize 
برای اینکه از kustomize برای دیپلوی کردن روی سه namespace ای که ساختیم استفاده کنم همچین ساختار فایلی رو ایجاد میکنم که تو مخزن گیتهابم موجود هستش و میتونید clone کنید 
```
.
|-- base
|   |-- kustomization.yaml
...
* هلم رو میشه به صورت چارت استفاده کرد و ورژن زد یعنی میشه ورژن های مختلفی از یه چارت داشت 
...
همون طوری که هلم و کاستومایز برای پاسخ دادن به نیاز کاربرایی ایجاد شدن که نیاز به آزادی عمل بیشتری برای دیپلویمنت های خودشون داشتند ArgoCD هم برای شرکت ها و سازمان ها و تیم ها واصولا جاهایی که نیاز به همکاری برای دیپلوی کردن اپلیکیشن ها دارن ایجاد شد 
...
این رو mindset همگی تاثیرگذاشت به صورتی که توافق کردیم که هرانچیزی که روی ریپازیتوری های ما هست باید تو پروداکشن هم باشد. 
...
ساده ترین مدل دیپلوی روی Argocd به وسیله Application که crd هستش اتفاق میفته برای مثال میخوام که پادی که با kubectl ساختم رو روی کلاستر stg اجرا کنم 
...
پالیستی دیفالت ArgoCD اینجوری که برای sync کردن تغییراتی که روی ریپازیتوری گیت خودتون دارید باید به صورت دستی دکمه sync رو بزنید ولی خب میشه این رو هم به حالت اتوماتیک تغییر داد 
...
> توجه کنید که حتما مانیفست رو روی نیم اسپیس argocd اعمال کنید در غیر این صورت argocd وقعی به ریسورسی که تو namespace های دیگه ایجاد کرده باشید نمیزاره 
